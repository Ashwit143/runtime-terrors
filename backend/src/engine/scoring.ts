...
import { calculateDistanceKm } from '../data/indianCities.js';
import {
  Listing,
  MatchScoreResult,
  FactorBreakdown,
...
export function calculateMaterialCompatibility(
  ...
  const weight = MATCHING_CONFIG.weights.materialCompatibility;
...
export function calculateTransportFeasibility(
  supplier: Listing,
  receiver: Listing
): { factor: FactorBreakdown; distanceKm: number } {
  const weight = MATCHING_CONFIG.weights.transportFeasibility;
  const distanceKm = calculateDistanceKm(supplier.city, receiver.city);
  const { breakpoints, maxDistanceFloorScore } = MATCHING_CONFIG.transportFeasibility;
...
export function calculateQuantityFit(
  supplier: Listing,
  receiver: Listing
): { factor: FactorBreakdown; quantityRatio: number } {
  const weight = MATCHING_CONFIG.weights.quantityFit;
  const cfg = MATCHING_CONFIG.quantityFit;
 ...
 */
export function calculateQualityMatch(
  supplier: Listing,
  receiver: Listing
): { factor: FactorBreakdown; qualityTierDiff: number } {
  const weight = MATCHING_CONFIG.weights.qualityMatch;
  const cfg = MATCHING_CONFIG.qualityMatch;
 ...
 */
export function calculateAvailabilityFrequency(
  supplier: Listing,
  receiver: Listing
): FactorBreakdown {
  const weight = MATCHING_CONFIG.weights.availabilityFrequency;
  const cfg = MATCHING_CONFIG.availabilityFrequency;
 ...
 */
export function scorePair(supplier: Listing, receiver: Listing): MatchScoreResult {
